...
def plot_with_labels(russelWord, emotionWord, filename='tsneTest.png'):
    ...
    #     y = float(line[2])
    #
    #     plt.scatter(x, y, color='black', s=15) # 마커, s=사이즈
    #     # 포인트에 대한 주석달기 (이름)
    #     plt.annotate(word, xy=(x, y),
    #              xytext=(0, 0), # 텍스트가 점이랑 떨어져있는 정도
    #              textcoords='offset points',
    #              fontproperties=font_manager.FontProperties(fname='c:/Windows/Fonts/NanumGothic.ttf', size=24),
    #              ha='right', va='bottom',
    ...
    #              textcoords='offset points', fontsize=10, fontproperties=emotionFont, ha='right', va='bottom',
    #              color='black', weight = "bold"
    #              # arrowprops=dict(arrowstyle="->")
    #     )

    # # 신조어 감성 표현 단어 배치
    # for line in coinedWord:
    #     if line[2] == '30':
    #         continue
    #
    #     word = line[0]
